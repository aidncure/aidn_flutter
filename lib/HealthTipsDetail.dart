...
class HealthTipsDetail extends StatefulWidget {
  ...
  HealthTipsDetail(int? id) {
    this.id = id;
  }

  @override
  _HealthTipsDetailState createState() => _HealthTipsDetailState(id);
}
...
class _HealthTipsDetailState extends State<HealthTipsDetail> {
  ...
  Future<BaseModel<HealthTipDetails>> healthtipdetail() async {
    HealthTipDetails response;
    setState(() {
      _loadding = true;
    });
    try {
      response = await RestClient(Retro_Api2().Dio_Data2()).healthtipdetailRequest(id);
      setState(() {
        _loadding = false;
