...
class MadicinePayment extends StatefulWidget {
  @override
  _MadicinePaymentState createState() => _MadicinePaymentState();
}

class _MadicinePaymentState extends State<MadicinePayment> {
                                        ...
                                        Container(
                                          ...
                                          child: deliveryType == 'Home'
                                              ? ElevatedButton(
                                                  ...
                                                  onPressed: () {
                                                    SharedPreferenceHelper.setString('isWhere', "MadicinePayment");
  ...
  Future<String> callApiBookMedicine() async {
    ...
    if (prescriptionFilePath == "") {
      formData = FormData.fromMap({
        ...
        "payment_token": _payment!.index == 5 ? "" : Payment_Token,
    ...
    try {
      var response = await dio.post("${Preferences.base_url}book_medicine", data: formData);
      // setState(()  {
  ...
  Future<BaseModel<PharamaciesDetails>> CallApipharamacyDetail() async {
    ...
    try {
      response = await RestClient(Retro_Api().Dio_Data()).pharmaciesdetailRequest(pharmacyId);
      setState(() {
        _loadding = false;
