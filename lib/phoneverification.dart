...
class phoneverification extends StatefulWidget {
  ...
  phoneverification(int? id) {
    this.id = id;
  }

  @override
  _phoneverificationState createState() => _phoneverificationState(id);
}
...
class _phoneverificationState extends State<phoneverification> {
  ...
  _phoneverificationState(int? id) {
    this.id = id;
  }
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          margin:
                              EdgeInsets.symmetric(horizontal: width * 0.0, vertical: width * 0.02),
                          child: Column(
                            children: [
                              GestureDetector(
                                onTap: () {
  ...
  Future<BaseModel<Checkotp>> CallApiOTP() async {
    ...
    try {
      response = await RestClient(Retro_Api2().Dio_Data2()).checkotp(body);
      setState(() {
  ...
  Future<BaseModel<ResendOtp>> callApiResendOtp() async {
    ResendOtp response;
    try {
      response = await RestClient(Retro_Api2().Dio_Data2()).resendOtpRequest(id);
      setState(() {
        if (response.success == true) {
          setState(() {
            Fluttertoast.showToast(
